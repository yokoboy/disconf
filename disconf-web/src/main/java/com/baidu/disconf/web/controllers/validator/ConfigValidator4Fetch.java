...
public class ConfigValidator4Fetch {
    ...
    public ConfigFullModel verifyConfForm(ConfForm confForm, boolean unCheckKey) {
        ...
        if (StringUtils.isEmpty(confForm.getEnv())) {
            throw new FieldException("envBO", "envBO is empty", null);
        }

        EnvBO envBO = envMgr.getByName(confForm.getEnv());
        if (envBO == null) {
            throw new FieldException("envBO", "envBO " + confForm.getEnv() + " doesn't exist in db.", null);
        }
        ...
        return new ConfigFullModel(app, envBO, confForm.getVersion(), confForm.getKey());
    }
}
